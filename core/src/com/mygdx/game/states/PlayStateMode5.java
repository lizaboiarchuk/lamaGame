...
public class PlayStateMode5 extends State{
    ...
    PlayStateMode5(GameStateManager gsm) {
        ...
        lama=new Lame(clouds.get(5).position.x+10, clouds.get(2).position.y+17);
    ...
    public void update(float dt) {
        ...
        for (int d=0; d<clouds.size(); d++)//repos the clouds when they are out of camera viewport
        {
           ...
           cl.moveable=true;
            if (d%2==0) {cl.yMoveable=true; cl.moveable=false;}
        ...
        for (Cloud c:clouds) {
            if ((lama.position.y<=c.position.y+21) && (lama.position.y>=c.position.y+15) && (lama.position.x+15>=c.position.x) && (lama.position.x<=c.position.x+Cloud.CLOUD_WIDTH-10)) {
                ...
                if ((lama.isOnCloud) && (c.yMoveable))
                    lama.position.y+=c.velocityY/2;
    ...
    public void render(SpriteBatch sb) {
        ...
        sb.draw(background, camera.position.x-camera.viewportWidth/2, camera.position.y-camera.viewportHeight/2, background.getWidth()+50,background.getHeight()+150);
